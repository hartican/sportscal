...
assert.match(html, /name:"Anticipation"/);
...
assert.match(html, /if \(state !== "compact"\)[\s\S]+buildEditorialSentiment\(ev\)/, "Sentiment must appear only at L1 and L2");
assert.match(html, /label\.textContent = "Sentiment"/);
