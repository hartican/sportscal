...
assert(html.includes('<span class="tab-label">Inspector</span>'), "top navigation must be named Inspector");
assert(!html.includes("Code Inspector"), "the retired Code Inspector label must not remain user-facing");
assert(html.includes('return "#inspector"') && html.includes("#inspect/") && html.includes("history.pushState") && html.includes("popstate"), "Inspector must use picker and detail history states with browser Back");
...
assert(html.includes('back.textContent = "Back to Inspector"'), "Inspector detail must return to the picker rather than directly to Feed");
...
assert(html.includes("Froth represents enthusiasm for a sport") && html.includes("Inspector always shows All Fixtures"), "the accessible Froth help must explain feed effects and Inspector isolation");
...
console.log(`Inspector UI contract valid across ${manifest.codes.length} canonical codes.`);
