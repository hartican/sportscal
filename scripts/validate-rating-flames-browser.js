...
(async()=>{const browser=await chromium.launch({headless:true,channel:process.env.QA_BROWSER_CHANNEL||'chrome'});try{for(const width of [320,390,768,1280]){
 ...
 for(const inDrawer of [false,true])for(const rating of [0,5]){
  ...
  assert(geometry.boxes.every(b=>b.svgWidth<=20&&b.svgHeight<=20),`${width}px: smaller visible flames`);
  assert(geometry.boxes.slice(1).every((b,i)=>b.left-geometry.boxes[i].right>=0),`${width}px: separate flames with a clear gap`);
  assert(geometry.boxes.every(b=>b.left>=geometry.card.left&&b.right<=geometry.card.right),`${width}px: controls fit the card`);
  assert.equal(geometry.disabled,false,'ratings remain editable within their phase');
