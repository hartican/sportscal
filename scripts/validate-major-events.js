...
assert.match(html, /const crowdEvent = majorSubEventNothingscoreEvent\(subEvent, record, fixture\);[\s\S]*registerNothingscoreEvent\(crowdEvent\);[\s\S]*const crowdHook = buildCrowdEditorialL0\(nothingscoreSnapshotFor\(crowdEvent\)\);[\s\S]*buildEditorialL0Hook\(editorialNarrativeHookForDisplay\(editorialRecord\), editorialConsequenceForDisplay\(editorialRecord\)\)/, "every Events timetable card must prefer registered crowd copy and retain validated sourced context as its fallback");
