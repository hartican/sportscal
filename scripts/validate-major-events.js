...
assert(!html.includes('<script src="config/major-events.js"></script>') && html.includes('moduleScriptUrl: "config/major-events.js?v=198"'), "the Events runtime must stay off the critical startup path and load with its catalogue");
...
assert(worker.includes('"/config/major-events.js?v=198"') && worker.includes('"/schemas/major-events.schema.json"'), "Events logic and schema must remain offline-capable");
...
assert.match(html, /const crowdEvent = majorSubEventNothingscoreEvent\(subEvent, record, fixture\);[\s\S]*registerNothingscoreEvent\(crowdEvent\);[\s\S]*row\.appendChild\(buildNothingscoreSummary\(crowdEvent\)\);[\s\S]*buildEditorialL0Hook\(editorialNarrativeHookForDisplay\(editorialRecord\), editorialConsequenceForDisplay\(editorialRecord\), \{ supplementalCopy:crowdEditorialSupplement\(crowdSnapshot\) \}\)/, "every Events timetable card must render registered NSC statistics independently from validated sourced context");
