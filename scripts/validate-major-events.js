...
assert(html.includes("canonicalEditorialBySemanticKey") && html.includes("editorialMatch?.editorialNarrative"), "a pinned Events child must inherit the curated projection when it is the only version eligible for Feed");
...
assert(!html.includes('<script src="config/major-events.js"></script>') && html.includes('moduleScriptUrl: "config/major-events.js?v=174"'), "the Events runtime must stay off the critical startup path and load with its catalogue");
...
assert(worker.includes('"/config/major-events.js?v=174"') && worker.includes('"/schemas/major-events.schema.json"'), "Events logic and schema must remain offline-capable");
