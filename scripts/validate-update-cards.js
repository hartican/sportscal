...
assert(localSteps.some(step => step[0] === "scripts/validate-feed-coverage-resilience.js"), "every update must reject stale follow artifacts, missing released tournament days and unexplained future gaps");
assert(localSteps.some(step => step[0] === "scripts/validate-quick-results-cadence.js"), "every update must prove the three Sydney quick-refresh windows across daylight-saving changes");
...
assert(localSteps.some(step => step[0] === "scripts/validate-fixture-editorial-resolution.js"), "every canonical update must keep manual pins and Events children bound to canonical fixture editorial or the research queue");
...
assert(localSteps.some(step => step[0] === "scripts/validate-preference-reset-recovery.js"), "every canonical update must enforce double-confirmed, service-only and reversible preference reset semantics");
...
assert.match(releaseScript, /vercel list "\$VERCEL_PROJECT" --meta "releaseGitSha=\$DEPLOY_SHA" --status READY --json/, "the release must query READY deployments by immutable source commit");
assert.match(releaseScript, /vercel project inspect "\$VERCEL_PROJECT" --scope "\$VERCEL_SCOPE" --token "\$VERCEL_TOKEN"/, "CI must validate a project-scoped token against the linked deployment target");
assert.match(releaseScript, /VERCEL_LIST_COMMAND=\(vercel list[^]*--token "\$VERCEL_TOKEN"\)/, "deployment verification must pass the saved token explicitly to Vercel CLI");
...
assert.match(snapshotScript, /run_vercel\(\)[^]*vercel "\$@" --token "\$VERCEL_TOKEN"/, "environment checks and immutable deployment must pass the saved token explicitly to Vercel CLI");
