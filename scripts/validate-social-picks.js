...
(async()=>{
 const available=[{kind:'entity',entityType:'team',id:'team:nrl:one',sport:'nrl'},{kind:'entity',entityType:'team',id:'team:nrl:muted',sport:'nrl'},{kind:'entity',entityType:'team',id:'team:afl:one',sport:'afl'},{kind:'entity',entityType:'athlete',id:'competitor:nrl:one',sport:'nrl'}];
 ...
 await social.copy({items:[available[3]]},{id:'actor'},deps);assert(writes[2].new_preferences.preferenceGraph.entityFollows.some(x=>x.participantId===available[3].id&&x.followLevel==='follow'),'individual athlete picks use the same additive copy path');
 const none=await social.copy({items:[]},{id:'actor'},deps);assert.equal(none.bonusAwarded,0);assert.equal(writes.length,3);
