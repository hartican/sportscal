...
(async()=>{
 const revisions=[...await allRows('nothingsports_predictions',{select:'event_id',result:'eq.pending'}),...await allRows('nothingsports_score_fixtures',{select:'event_id',status:'not.in.(completed,finished,final,cancelled,canceled,abandoned)'})];let total=0;
 ...
 if(!end)continue;
 total+=Number(await supabaseServiceRequest('/rest/v1/rpc/nothingsports_resolve_predictions',{method:'POST',body:{target_event_id:id,confirmed_end:end,final_status:event.status}}))||0;
 }
