...
assert.match(html, /visualViewport\?\.addEventListener\("scroll", syncHeight\)/, "the chat sheet must follow the iOS visual viewport as the keyboard pans it");
assert.match(html, /--chat-viewport-top/);
assert.match(html, /\.chat-room-composer\{[\s\S]*position:sticky;[\s\S]*bottom:0;/, "the room composer must stay pinned inside the keyboard-sized chat surface");
