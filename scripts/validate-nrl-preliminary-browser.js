...
(async()=>{const browser=await chromium.launch({channel:'chrome'});try{for(const width of [390,1280]){
 ...
 for(const [number,name,time,venue] of [[2,'Dolphins v Roosters','7:50 PM','LANG PARK'],[1,'Panthers v Knights','4:00 PM','STADIUM AUSTRALIA']]){
  ...
  const card=page.locator(`[data-feed-event-id="${id}"]`);await card.scrollIntoViewIfNeeded();await page.waitForTimeout(100);assert.equal(await card.count(),1,name+' appears once');
  const text=await card.innerText();for(const team of name.split(' v '))assert(text.includes(team),name+' named sides');assert(text.includes(time));assert(text.toUpperCase().includes(venue),text);assert(text.includes('PRELIM'));assert(!/Teams.*pending|winner of|Venue TBC/i.test(text));
