...
assert.match(commsUi,/manual-content-handoff\.v2/);
assert.match(commsUi,/Copy complete handoff/);
assert.match(commsUi,/action:"autosave"/);
assert.match(commsUi,/setTimeout\(\(\)=>save\(\).*800/);
assert.match(commsUi,/Primary CTA label/);
assert.match(commsUi,/Image alt text/);
assert.match(commsUi,/Send date pending/);
assert.match(commsUi,/campaign\.candidate\?\.readyForExport===false\?"Watching"/);
assert.match(commsUi,/Publish live revision/);
assert.match(commsUi,/Upload approved media/);
assert.match(commsUi,/Copy handoffs/);
assert.match(commsUi,/Copy Hootsuite handoff/);
assert.doesNotMatch(commsUi,/Save selected|Approve \+ export selected|Prepare Mailchimp export/);
...
assert.match(worker,/"\/config\/admin-comms-workspace\.js\?v=217"/);
