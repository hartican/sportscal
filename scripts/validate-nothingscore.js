...
assert.match(api, /function accountUser\(user\)\{return user&&user\.is_anonymous!==true\?user:null\}/, "anonymous chat identities must not be treated as NSC account users");
const server = read("lib/nothingscore-server.js");
const enrichmentSource = read("config/enrichment-engine.js");
...
require("./app-shell-test-utils").assertShellModule(html,"config/enrichment-engine.js");
assert.doesNotMatch(html, /<script src="config\/nothingscore\.js"><\/script>/);
assert.match(enrichmentSource, /browserNothingscore[\s\S]+NOTHINGSPORTS_NOTHINGSCORE/);
const browserContext = { globalThis:{} };
vm.runInNewContext(enrichmentSource, browserContext);
const browserNsc = browserContext.globalThis.NOTHINGSPORTS_NOTHINGSCORE;
assert.deepEqual(Array.from(browserNsc.HEAT_LABELS), Array.from(nsc.HEAT_LABELS));
...
assert.match(html, /<span class="nothing-score-label" aria-hidden="true">Nothing<br>Score<\/span>/, "the compact header label must show Nothing Score on two lines");
assert.match(html, /<h2 id="nothingscoreTitle">Nothing Score<\/h2>/, "the drawer title must use the Nothing Score name");
