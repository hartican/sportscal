...
assert.match(html, /const nationalTeamIdentityReady[\s\S]*loadDeferredScript\("config\/national-team-identities\.js\?v=163"\)/, "the national registry must load concurrently with the first feed page through a shell-versioned URL");
...
assert.match(serviceWorkerSource, /nothingsport-shell-v163/, "the application-shell cache must be bumped for the identity library");
assert.match(serviceWorkerSource, /"\/config\/national-team-identities\.js\?v=163"/, "the offline shell must cache the exact versioned registry URL used at runtime");
