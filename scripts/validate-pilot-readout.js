...
assert.match(sql, /negative_feedback_by_sport/i, "the aggregate operator readout must segment existing left-swipes by sport");
assert.match(sql, /negative_feedback_by_competition/i, "the aggregate operator readout must segment existing left-swipes by competition");
assert.match(sql, /'pending_approval'::text as instrumentation_status/i, "unapproved discovery action fields must remain explicitly pending");
