...
(async()=>{
 ...
 const deps={api:{TABLES:{profiles:'profiles'},rows:async(_table,params)=>params.visibility&&!visible?[]:[target],personaFor:async()=>({moderation_flag:moderated})},request:async(path,options)=>{writes.push({path,options});return {targetProfileId:target.profile_id,following:options.body.following};}};
 ...
 assert.equal(writes[0].options.body.actor,actor.id,'actor comes from verified session');
 assert.equal(writes[0].options.body.target_profile,target.profile_id);
 visible=false;await setFollow({...body,following:false},actor,deps);
 assert.equal(writes[1].options.method,'POST');assert.equal(writes[1].options.body.following,false);
