...
(async()=>{const browser=await chromium.launch({headless:true,channel:process.env.QA_BROWSER_CHANNEL||'chrome'});try{for(const width of [320,390,768,1280]){
 ...
 const e={profileId:'11111111-1111-4111-8111-111111111111',name:'Sports Friend With A Very Long Display Name',handle:'sports_friend_long_handle',rank:1,points:41,eligible:4,efficiency:.75,fixtures:5,following:true,canFollow:true,sports:{nrl:{points:20,eligible:2,efficiency:1,rank:1},tennis:{points:21,eligible:2,efficiency:.5,rank:2}}};
 ...
 await page.getByRole('tab',{name:'Global Leaderboard'}).waitFor();assert.equal(await page.locator('#nothingscoreTitle').textContent(),'Nothinger Leaderboard');if(width>=768){await page.getByRole('button',{name:'Next sport'}).click();await page.getByRole('columnheader',{name:/AFL/}).waitFor();}else{const detail=page.locator('.nsc-ladder-viewport .nsc-mobile-statistics').first();await detail.locator('summary').click();assert.match(await detail.innerText(),/Resolved predictions/);assert.equal(await page.locator('#nothingscoreBody').evaluate(el=>el.scrollWidth-el.clientWidth),0);}
