...
const AFL_STORIES = Object.freeze({
  "event-afl-cd_m20260142701": { hook:"Fremantle's minor premiership now offers no safety net: Geelong brings its September run west for a single remaining path to a preliminary final.", synopsis:"Nineteen home-and-away wins put Fremantle first, while Geelong entered September fifth after six consecutive wins. The published semi-final brings those different routes together in Perth. Both clubs now need this match to keep their premiership campaign alive; the double chance is gone.", facts:["Fremantle finished the home-and-away season first with 19 wins.", "Geelong finished fifth after six consecutive home-and-away wins.", "The official week-two fixture pairs Fremantle and Geelong in a semi-final in Perth; there is no second chance in a semi-final."], dims:["form","path","consequence"], thread:"finals" },
  "event-afl-cd_m20260142702": { hook:"Brisbane's top-three season earns one more home opportunity; Adelaide must turn its sixth-place route into an away semi-final breakthrough.", synopsis:"Brisbane entered September third and Adelaide sixth. Their published week-two meeting puts the Lions' home advantage against a Crows campaign that already began on the elimination side of the bracket. A preliminary-final place is the prize, with neither team retaining a second chance.", facts:["Brisbane finished the home-and-away season third; Adelaide finished sixth.", "The official week-two fixture pairs Brisbane and Adelaide in Brisbane.", "A semi-final has no second chance and leads to a preliminary-final place."], dims:["path","venue","consequence"], thread:"finals" },
  ...
  "event-afl-cd_m20260142502": { hook:"The first AFL wildcard final came down to one kick after the siren; the survivor is hidden until spoilers are enabled.", synopsis:"Western Bulldogs brought an eight-wins-in-11 recovery into the first wildcard final, while Collingwood's pressure game had rediscovered features of its premiership identity. The new Final Ten compressed both arcs into one elimination night, ultimately separated by a shot after the siren.", facts:["The Bulldogs won eight of 11 matches during their climb back into finals contention before the wildcard round.", "The first wildcard final was decided by a shot after the siren, with the two teams separated by three points.", "The wildcard was sudden-death: the winner advanced to an elimination final and the loser left the premiership race."], dims:["form","history","consequence"], thread:"finals" },
  "event-afl-cd_m20260142501": { hook:"Carlton's recovery from a one-and-eight start met Melbourne's surprise finals season; the advancing side remains concealed here.", synopsis:"Melbourne's first season under Steven King rose from low expectations into finals contention. Carlton entered the same knockout after Josh Fraser inherited one positive result from nine matches and drove a late charge, making the wildcard a collision between an early-season rise and a late-season rescue.", facts:["Carlton recovered from a 1-8 start to reach the inaugural wildcard round under Josh Fraser.", "Melbourne reached finals contention in Steven King's first season after exceeding early expectations.", "The wildcard was sudden-death: the winner advanced to an elimination final and the loser left the premiership race."], dims:["form","history","consequence"], thread:"finals" },
  "event-afl-cd_m20260142601": { hook:"Minor premier Fremantle meets the Hawthorn side that seized fourth on the final day; the reward is a week off, not elimination.", synopsis:"Fremantle's 19-win season earned the first qualifying final at Optus Stadium. Hawthorn arrived after its final-round performance delivered the last top-four place, so this became a contest between season-long control and a late seeding surge, with a direct home preliminary-final place at stake.", facts:["Fremantle finished first with 19 wins and earned a home qualifying final at Optus Stadium.", "Hawthorn secured fourth place through the final match of Round 24, entering the double-chance side of the bracket.", "The qualifying-final winner advances directly to a home preliminary final, while the loser retains a second chance in a semi-final."], dims:["form","path","consequence"], thread:"finals" },
  "event-afl-cd_m20260142603": { hook:"Carlton's rescue from a one-win, eight-loss start has one life left against a Geelong side that closed the season with six straight wins.", synopsis:"Carlton carried its comeback through the wildcard round and now meets the form team of the run-in. Geelong's six-match winning finish secured fifth and an MCG elimination final; the contrast is a long recovery against late-season continuity, with no second chance for either.", facts:["Geelong ended the home-and-away season with six consecutive wins and finished fifth.", "Carlton advanced from 10th through the wildcard round after recovering from a 1-8 start.", "The elimination final has no second chance: its winner reaches the semi-finals and its loser is eliminated."], dims:["form","path","consequence"], thread:"finals" },
  "event-afl-cd_m20260142602": { hook:"Sydney's five straight wins meet Brisbane's three in a qualifying final that turns current form into a week-off prize.", synopsis:"Sydney entered September second after five consecutive wins; Brisbane entered third with three of its own. The SCG qualifying final matches the two strongest finishing streaks on this side of the bracket, with a week off and a home preliminary final on offer and a second chance for the other side.", facts:["Sydney enters the finals on a five-match winning streak after finishing second.", "Brisbane enters on three consecutive wins after finishing third.", "The qualifying-final winner advances directly to a home preliminary final, while the loser retains a second chance in a semi-final."], dims:["form","path","consequence"], thread:"finals" },
  "event-afl-cd_m20260142604": { hook:"The Bulldogs survived the first wildcard by one kick; Adelaide now asks them to win sudden-death football away from home.", synopsis:"Western Bulldogs arrived in Adelaide after the inaugural wildcard was decided by an after-the-siren miss. The Crows earned sixth and the home elimination final, turning this into a test of whether wildcard momentum travels against a side whose whole season secured this venue advantage.", facts:["Western Bulldogs advanced from the first wildcard final by three points after Collingwood missed after the siren.", "Adelaide finished sixth and earned the right to host the elimination final at Adelaide Oval.", "The elimination final has no second chance: its winner reaches the semi-finals and its loser is eliminated."], dims:["form","path","consequence"], thread:"finals" },
  "event-afl-cd_m20260142901": { hook:require("../config/editorial-locks").records[0].hook, synopsis:require("../config/editorial-locks").records[0].synopsis, facts:["The 2026 season is the first AFL campaign to use a Final Ten and wildcard round.", "The two preliminary-final winners advance to a single Grand Final at the MCG.", "The Grand Final is the premiership decider, so its winner becomes the 2026 AFL premier and there is no second chance.", "The 2026 AFL Grand Final is scheduled at the Melbourne Cricket Ground."], dims:["history","path","consequence","venue"], thread:"finals" },
});
...
function buildAfl(knowledge, events, reference){
  ...
  const activeTargetIds = new Set(targetEvents.map(idFor));
  knowledge.eventProjections = knowledge.eventProjections.filter(projection => !(
    projection.targetType === "feed-event"
    && (projection.targetIds || []).some(targetId => AFL_STORIES[targetId] && !activeTargetIds.has(targetId))
  ));
  for (const event of targetEvents){
    const approved=require("../config/editorial-locks").recordFor(event);
    const story = {...AFL_STORIES[idFor(event)],...(approved ? {hook:approved.hook,synopsis:approved.synopsis} : {})};
...
function main(){
  ...
  knowledge.eventProjections = knowledge.eventProjections.filter(projection => !(
    ...
    && !(projection.targetIds || []).some(targetId => publishedFeedIds.has(String(targetId)))
  ));
