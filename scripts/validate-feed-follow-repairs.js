...
const published=require('../data/tournament-horizon.v1.json');assert.equal(published.through,horizon.add(published.from,27));for(const event of published.tournaments){assert(horizon.inHorizon(event,published.from));assert(event.formatConfirmed||event.slots.length===0,"unknown formats must not invent slots");assert.equal(new Set(event.slots.map(s=>s.slotId)).size,event.slots.length);}
...
for(const t of published.tournaments){assert(t.formatConfirmed||t.slots.length===0,`${t.name}: unpublished formats must remain a concise pending state`);assert(/^https:\/\//.test(t.formatSourceUrl),`${t.name}: format needs a source`);}
