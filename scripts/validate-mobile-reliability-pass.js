...
assert(html.includes("const verb=viewingLink.liveOrReplay==='replay'?'Replay':'Watch';") && html.includes("link.title=`${verb} on ${viewingLink.label}`;link.setAttribute('aria-label',link.title)"), "Feed and Events viewing actions must put the accessible Watch / Replay labels on provider marks");
