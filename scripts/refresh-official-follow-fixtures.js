...
function isTransientSourceFailure(error){
  return /fetch failed|timed?\s*out|abort|network|socket|econn|enotfound|eai_again|Official follow source failed \((?:404|408|425|429|5\d\d)\)/i.test(String(error?.message || error));
}
