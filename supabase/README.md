...
9. On Team or Enterprise, optionally enable the **Before User Created** Postgres hook and select `public.nothingsports_before_user_created`. Supabase does not expose this hook on the Free plan. Free production therefore relies on the signed room capability, server-side per-room/IP rate gate, atomic room join, guest attestation and deny-by-default chat RLS. Unattested anonymous identities cannot write user state, metadata or telemetry and are removed by the bounded daily orphan cleanup.
10. In Authentication > Sign In / Providers, enable global account creation, anonymous sign-ins and Email sign-in, and keep email confirmation enabled. On Free, the global signup switch cannot close Email signup while retaining anonymous guest signup. Enable CAPTCHA when its client challenge is integrated; do not enable it before the server guest flow can supply a valid challenge token.
