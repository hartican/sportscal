...
sports as(select s.*,row_number() over(partition by sport order by points desc,efficiency desc nulls last,eligible desc,p.profile_id) rank from public.nothingsports_sport_stats s join people p using(user_id) where sport<>'overall'),
