...
function participationHistory(events,participantId){
 const history=[];
 for(const event of events){
  const fixture=event.fixtureFallback||event;
  for(const entry of event.participationEvidence||[]){
   if(entry.participantId!==participantId||!/^https:\/\//.test(entry.sourceUrl||''))continue;
   const status={confirmed:'Confirmed entry',provisional:'Provisional entry',completed:'Participated',withdrawn:'Withdrawn'}[entry.participationStatus];if(!status)continue;
   history.push({eventId:fixture.id,date:fixture.date,discipline:fixture.discipline||fixture.competitionName||fixture.key,kind:entry.participationKind||'race',participationStatus:entry.participationStatus,description:`${status} — ${fixture.name}`,sourceUrl:entry.sourceUrl});
  }
 }
 return [...new Map(history.map(item=>[item.eventId,item])).values()].sort((a,b)=>String(b.date).localeCompare(String(a.date)));
}
module.exports={materializeParticipation,participationHistory};
