...
const SERVER_FEED_BUILD_VERSION = "direct-entity-follow.v4";
...
function shouldEnrichEvent(event, preferences, action, expandedEntityIds = new Set(), now = new Date()){
  ...
  if(includeMajorEvents===false && event.majorEventId)return false;
  return followFeedPolicy.eligibleForFollow(event,{competitionFollow:true});
}
...
function buildServerFeed({
  ...
  const personalisedEvents = eligibleRetainedEvents
    .slice()
    .sort((first, second) => {
      const bucketOrder = chronologicalBucket(first, reference) - chronologicalBucket(second, reference);
      if (bucketOrder) return bucketOrder;
      const firstFollowed = participantFollowLevels(first, preferences.preferenceGraph, expandedEntityIds)
        .some(level => level === "follow" || level === "priority");
      const secondFollowed = participantFollowLevels(second, preferences.preferenceGraph, expandedEntityIds)
        .some(level => level === "follow" || level === "priority");
      if (firstFollowed !== secondFollowed) return firstFollowed ? -1 : 1;
      const firstExact = Boolean(eventStart(first));
      const secondExact = Boolean(eventStart(second));
      if (firstExact !== secondExact) return firstExact ? -1 : 1;
      return comparePersonalisedChronology(first, second, reference);
    });
