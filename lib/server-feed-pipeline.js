...
const SERVER_FEED_BUILD_VERSION = "follow-policy.v10";
...
function normalizeUserFollowState(userState){
  ...
  const migrated = discoveryCatalogue.migratePreferences(plainObject(state.preferences));
...
function shouldEnrichEvent(event, preferences, action, expandedEntityIds = new Set(), now = new Date(), preparedPreferences = null){
  ...
  const muted=followFeedPolicy.participantIds(event).some(id=>followFirst.effectiveParticipantFollow(id,preferences,{},preparedPreferences).source==="mute");
  if (cardLifecycle.isRetentionExemptAction(action) && !muted) return true;
  const graph = plainObject(preferences.preferenceGraph);
...
function buildServerFeed({
  ...
  const normalizedEvents = events.map(event => normalizeEvent(event, reference, {copyEvents})).filter(event => event.eventId);
