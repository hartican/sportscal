...
function buildServerFeed({
  ...
  onTiming = () => {},
  copyEvents = true,
  tennisProjection = tennisParents,
  matchCentreOnly = false,
} = {}){
  ...
  events=[...events,...(tennisProjection.contests||[]).filter(e=>!knownIds.has(eventId(e)))];
  const parents = (tennisProjection.parents||[]).map(parent=>tennisFeed.reconcile(parent,events));
  ...
  const personalisedEvents = eligibleRetainedEvents
    .filter(event=>!matchCentreOnly || (!actionFor(event).archived&&!actionFor(event).dismissed&&require('../config/match-centre').eligible(event,+reference)))
