...
const SERVER_FEED_BUILD_VERSION = "follow-policy.v9";
...
function shouldEnrichEvent(event, preferences, action, expandedEntityIds = new Set(), now = new Date()){
  ...
  if (!followFeedPolicy.sportingFixture(event) || followFeedPolicy.explicitlyExcluded(event,preferences)) return false;
  if (cardLifecycle.isRetentionExemptAction(action) && !followFeedPolicy.participantIds(event).some(id=>followFirst.effectiveParticipantFollow(id,preferences).source==="mute")) return true;
  const graph = plainObject(preferences.preferenceGraph);
  if(followFeedPolicy.participantIds(event).some(id=>followFirst.effectiveParticipantFollow(id,preferences).source==="mute"))return false;
  ...
  return Boolean(followFirst.reasonForEvent(event, preferences));
}
