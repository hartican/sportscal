...
function participantFollowLevels(event, preferenceGraph){
  ...
  const expanded = footballDirectory.expandedFollowLevels(event, preferenceGraph, footballFollowIndex);
...
function isCoreLeagueFootball(event){
  return [
    ...
    "competition:la-liga", "competition:serie-a", "competition:ligue-1", "competition:a-leagues",
...
function buildServerFeed({
  ...
  const actionFor = event => {
    const action = eventActionFor(event, actions, actionIndex);
