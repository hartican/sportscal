...
const SERVER_FEED_BUILD_VERSION = "editorial-alias-dedupe.v1";
...
function chronologicalBucket(event, now){
  const start = eventStart(event);
  if (!start) return 3;
  if (sydneyDateKey(start) === sydneyDateKey(now)) return 0;
...
function comparePersonalisedChronology(first, second, now = new Date()){
  ...
  const chronology = firstBucket === 2
...
function shouldEnrichEvent(event, preferences, action, expandedEntityIds = new Set()){
...
function buildServerFeed({
  ...
  const eligibleRetainedEvents = retainedEvents.filter(event => shouldEnrichEvent(event, preferences, actionFor(event), expandedEntityIds));
  const enrichmentCandidates = retainedEvents.filter(event => {
    ...
    return (
      ...
      && shouldEnrichEvent(event, preferences, actionFor(event), expandedEntityIds)
    );
  });
