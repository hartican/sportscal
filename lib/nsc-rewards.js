...
async function submit(userId,eventId,phase,rating,event,now=new Date()){
  ...
  if(phase==='heat'&&!exact){
    return supabaseServiceRequest('/rest/v1/rpc/nothingsports_nsc_record_unconfirmed_opinion',{method:'POST',body:{target_user_id:userId,target_event_id:eventId,target_rating:rating,submitted_time:now.toISOString()}});
  }
