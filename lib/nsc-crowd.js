...
function latestVotes(rows,{phase,now=new Date(),excludeUser=null,cutoff=null}={}){
  ...
  for(const row of rows || []){
    ...
    if(phase==='pulse' && +now-time>=LIVE_MS)continue;
...
function summary(rows,options){
  const votes=latestVotes(rows,options),count=votes.length,raw=count?votes.reduce((sum,row)=>sum+row.rating,0)/count:null;
  return {phase:options.phase,count,average:raw===null?null:Math.round(raw*10)/10,rawAverage:raw,early:count>0&&count<5,ranked:count>=5,source:'real-users',excludesViewer:false,calculatedAt:new Date(options.now || Date.now()).toISOString()};
}
