...
function parse(html,{previous=[],checkedAt=new Date().toISOString()}={}){
 ...
 const golfer=require('./golf-participation').participant;
 const roster=new Map((data('tourCupCombined')?.players||[]).map(p=>[String(p.id),p]));
 const entries=new Map(),appearances=[];
 for(const round of tee?.rounds||[])for(const match of round.matches||[]){
  const sides=(match.teams||[]).map(team=>({name:team.teamName,participants:(team.players||[]).map(p=>golfer({...roster.get(String(p.playerId)),...p},{sourceUrl:URL,checkedAt}))}));
  const players=sides.flatMap(s=>s.participants);players.forEach(p=>entries.set(p.id,p));
  if(!players.length)continue;
  const start=Number.isFinite(match.teeTime)&&match.teeTime>0?new Date(match.teeTime).toISOString():null;
  appearances.push({id:`${id}:round:${round.roundNum}:match:${match.matchId}`,round:round.roundNum,label:round.roundDisplay,matchTitle:match.matchTitle,sides,participants:players,participantIds:players.map(p=>p.id),startTimeUtc:start,timePrecision:start?'exact':'tbc',tee:match.startingTee||null,status:match.matchStatus==='-'?'upcoming':match.matchStatus,score:match.matchStatus==='-'?null:match.displayScore,sourceUrl:URL,sourceCheckedAt:checkedAt});
 }
 if(entries.size){base.entries=[...entries.values()];base.participants.push(...entries.values());base.participantIds.push(...entries.keys());base.participantsConfirmed=true;base.appearances=require('./golf-participation').mergeObservation({appearances,participantsConfirmed:true},prior).appearances;}
