...
async function ladder({cursor=0}={},user=null,{request=supabaseServiceRequest}={}){
  ...
  const following=await require('./user-follows').followingProfileIds(user,{request});
  payload.entries=(payload.entries||[]).map(entry=>({...entry,following:following.has(entry.profileId),canFollow:Boolean(user)&&!entry.isViewer}));
