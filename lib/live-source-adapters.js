...
function liveSources(fetchImpl=globalThis.fetch,{environment=process.env}={}){
  ...
  sources.push({id:'live-bjk-cup',seed:require('../data/canonical/tennis-team-contests.v1.json').fixtures.filter(f=>f.tournamentId==='tournament:tennis:bjk-cup-finals-2026'),fetch:args=>require('./bjk-live').refresh({...args,fetchImpl})});
  sources.push({id:'live-presidents-cup',seed:require('../data/canonical/pga-tour-schedule.json').presidentsCup||[],fetch:args=>require('./presidents-cup').refresh({...args,fetchImpl})});
