...
function liveSources(fetchImpl=globalThis.fetch){
  ...
  const sources=[...coverageSources(fetchImpl)];
  for(const code of ["afl","aflw"]){
    sources.push({id:`live-${code}`,seed:seed.filter(event=>event.key===code),fetch:async({now,previous})=>{
      ...
      if(!near.length)return previous;
      ...
      const rounds=[...new Set(near.map(event=>event.roundNumber).filter(Number.isInteger))].slice(0,4),events=[];
      for(const round of rounds){
        ...
        events.push(...response.matches.map(match=>canonical.buildAflEvent(match,now.toISOString(),new Map(),code==="aflw"?{code,competitionId:"competition:aflw-2026",discoverySportId:"sport:aflw"}:{}).event));
      }return events;
    }});
  }
  sources.push({id:"live-nrl",seed:seed.filter(event=>event.key==="nrl"),fetch:async({now})=>{
    ...
    return response.fixture.match.map(match=>canonical.buildNrlEvent(match,now.toISOString(),new Map()).event);
  }});
