...
function collectionFollows(userState){
  const preferences = userState?.preferences || userState || {};
  const followFirst = preferences.followFirst || preferences.follow_first || {};
  return unique(Array.isArray(followFirst.collectionFollows)
    ? followFirst.collectionFollows
    : Array.isArray(followFirst.collection_follows) ? followFirst.collection_follows : []);
}

function followedCollectionMemberIds(userState, collections = tennisWatchPool.collections || []){
  const collectionsById = new Map((collections || []).map(collection => [collection.id, collection]));
  return unique(collectionFollows(userState).flatMap(collectionId => {
    const collection = collectionsById.get(collectionId);
    if (!collection) return [];
    const members = [...(collection.memberIds || [])];
    if (collection.includeTour){
      members.push(...(tennisContext.participants || [])
    ...
    if (collection.includeWatchPoolGender){
      members.push(...(tennisWatchPool.players || [])
...
function playerTeamEntries(entityIds = []){
  ...
  const needsNblPlayers = entityIds.some(entityId => entityId.startsWith("competitor:nbl:") || entityId.startsWith("athlete:nbl:"));
  const nblPlayers = needsNblPlayers ? require("../data/canonical/nbl-directory.v1.json").players || [] : [];
  return [
    ...(footballFollowIndex.players || []),
    ...(nrlFollowIndex.players || []),
    ...(aflFollowIndex.players || []),
    ...(aflwFollowIndex.players || []),
    ...f1Players,
    ...nflPlayers,
    ...hockeyPlayers,
    ...nblPlayers,
...
function expandedFollowEntityIds(userState,{event}={}){
  const excluded=new Set(event?.excludedParticipantIds || []);
  const follows = entityFollows(userState).filter(follow=>!excluded.has(follow.participantId));
  const identityKey=require("../config/follow-first").participantFollowIdentityKey;
  const mutedKeys=new Set(follows.filter(follow=>["unfollow","mute"].includes(follow.followLevel)).map(follow=>identityKey(follow.participantId)));
  const mutedIds = new Set(follows.filter(follow => ["unfollow","mute"].includes(follow.followLevel)).map(follow => follow.participantId));
  [...mutedIds].forEach(entityId => {
    const tennisAlias = entityId.match(/^competitor:tennis:(?:atp|wta):(.+)$/)?.[1];
    if (tennisAlias) mutedIds.add(`athlete:tennis:${tennisAlias}`);
  });
  const activeIds = new Set([
    ...follows.filter(follow => ["follow", "priority"].includes(follow.followLevel)).map(follow => follow.participantId),
    ...followedCollectionMemberIds(userState),
  ].filter(id=>!excluded.has(id)&&!mutedKeys.has(identityKey(id))));
  [...activeIds].forEach(entityId => {
    const tennisAlias = entityId.match(/^competitor:tennis:(?:atp|wta):(.+)$/)?.[1];
    if (tennisAlias && !mutedIds.has(entityId) && !mutedIds.has(`athlete:tennis:${tennisAlias}`)) activeIds.add(`athlete:tennis:${tennisAlias}`);
  });
  const playerTeams = new Map(playerTeamEntries([...activeIds]).filter(player => player?.id && player?.currentTeamId).map(player => [String(player.id), String(player.currentTeamId)]));
  [...activeIds].forEach(entityId => {
    const currentTeamId = playerTeams.get(entityId);
    if (currentTeamId && !excluded.has(currentTeamId) && !mutedIds.has(currentTeamId)) activeIds.add(currentTeamId);
  });
...
function footballBundlesForEntities(entityIds){
  ...
  return leagueIds.flatMap(leagueId => FOOTBALL_LEAGUE_FILES[leagueId]?.().events || []);
}
...
function sydneyParts(iso){
  const parts = SYDNEY_PARTS.formatToParts(new Date(iso)).reduce((result, part) => {
...
function normalizeTennisSubEvent(subEvent){
  ...
  if (!subEvent?.id || !subEvent?.date) return null;
  ...
  return {
    ...
    parentEventId:"major-event:us-open-2026",
    majorEventId:"major-event:us-open-2026",
    identityRef:"event:us-open",
    displayTitleCompact:subEvent.name,
    date:local.date,
    time:local.time,
    startTimeUtc:exactTime ? start.toISOString() : null,
    timelineSortTimeUtc:start.toISOString(),
    sessionStartTimeUtc:subEvent.sessionStartTimeUtc || (exactTime ? start.toISOString() : null),
    timePrecision:subEvent.timePrecision || (exactTime ? "exact" : "follows"),
    endTimeUtc:exactTime ? new Date(start.getTime() + 5 * 60 * 60 * 1000).toISOString() : null,
    timeTbc:!exactTime,
    ...
    liveWindow:3,
    storyline:subEvent.storyline || null,
    ...(subEvent.status === "completed" && subEvent.scoreDisplay ? {score:subEvent.scoreDisplay,outcomeText:subEvent.scoreDisplay,recapText:subEvent.editorialNarrative?.synopsisSpoilerOn || subEvent.scoreDisplay,resultStatus:"official"} : {}),
    sourceCheckedAt:subEvent.timingSource?.checkedAt || subEvent.statusUpdatedAt || subEvent.resultPublishedAt,
    sourceName:"US Open official schedule",
    sourceUrl:subEvent.sourceUrl,
    sourceType:"official",
    sourceTrust:"verified",
    venueCountryCode:"US",
    timingSource:subEvent.timingSource || {publisher:"US Open",url:subEvent.sourceUrl || null,checkedAt:subEvent.updatedAt || null,precedence:"official-tournament"},
  };
}
...
function participantRecords(entityIds){
  if(participantIndex)return [...entityIds].flatMap(id=>participantIndex.get(id)||[]).sort((a,b)=>a.order-b.order).map(item=>item.record);
  ...
  const aflw = [...(aflwFollowIndex.teams || []), ...(aflwFollowIndex.players || [])];
  ...
  [...football, ...nrl, ...afl, ...aflw, ...(f1Context.participants || []), ...tennis].forEach((record,order)=>{
    const id=String(record.id);if(!participantIndex.has(id))participantIndex.set(id,[]);
    participantIndex.get(id).push({record,order});
  });
  return participantRecords(entityIds);
}

function resolveUserFollowFixtures({ events = [], userState, includeCompactArtifact = true, copyEvents = true } = {}){
  const entityIds = expandedFollowEntityIds(userState);
  const followFirst=require('../config/follow-first');
  const preparedPreferences=followFirst.migratePreferences(userState?.preferences || {});
  ...
  const sourceEvents = [
    ...require("./competition-fixtures").fixtures(),
    ...require("./golf-fixtures").fixtures(require("../data/canonical/pga-tour-schedule.json")),
  ...
  ].filter(event => eventMatchesEntities(event, entityIds) || followFirst.reasonForEvent(event,userState?.preferences || {},{preparedPreferences}));
  ...
  [...events, ...sourceEvents].forEach(event => {
    ...
    if (existingId){
      ...
      const incoming = copyEvents ? clone(event) : event;
      const incomingHasEditorial = Boolean(incoming?.editorialNarrative?.projectionId);
      const existingHasEditorial = Boolean(existing?.editorialNarrative?.projectionId);
      const combined = incomingHasEditorial && !existingHasEditorial
        ? { ...existing, ...incoming }
        : { ...incoming, ...existing };
      if(incoming.entries?.length&&Date.parse(incoming.participationCheckedAt||incoming.sourceCheckedAt)>=Date.parse(existing.participationCheckedAt||existing.sourceCheckedAt||'1970-01-01')){
        for(const key of ['participants','participantIds','participantsConfirmed','excludedParticipantIds','entries','appearances','participationCheckedAt'])if(incoming[key]!=null)combined[key]=incoming[key];
      }
      combined.sourceEventIds = unique([existingId, eventId(existing), eventId(incoming), ...(existing?.sourceEventIds || []), ...(incoming?.sourceEventIds || [])]);
      merged.set(existingId, combined);
      return;
    }
    merged.set(id, copyEvents ? clone(event) : event);
