...
function semanticFixtureKey(event){
  ...
  const start = String(event?.startTimeUtc || (event?.date && event?.time ? `${event.date}T${event.time}` : ""));
  const competition = String(event?.competitionId || event?.key || event?.sport || "");
  if (participantIds.length < 2 || !start || !competition) return "";
  return `${competition}|${start}|${participantIds.join("|")}`;
}
...
function resolveUserFollowFixtures({ events = [], userState, includeCompactArtifact = true } = {}){
  ...
  if (!entityIds.size) return { events:clone(events), participants:[], entityIds:[], sourceVersion:FOLLOW_FIXTURE_VERSION };
