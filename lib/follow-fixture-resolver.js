...
function participantRecords(entityIds){
  ...
  return [...football, ...nrl, ...afl, ...aflw, ...(f1Context.participants || []), ...tennis]
...
function resolveUserFollowFixtures({ events = [], userState, includeCompactArtifact = true } = {}){
  ...
  ].filter(event => eventMatchesEntities(event, entityIds) || require("../config/follow-first").reasonForEvent(event,userState?.preferences || {}));
