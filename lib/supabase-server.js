...
function userStateFromRow(row){
  ...
  return {
    ...
    eventUserState: plainObject(source.eventUserState || source.event_user_state),
...
function mergeUserState(existing, incoming){
  ...
  return {
    ...
    preferences: mergeSettings(previous.preferences, patch.preferences),
    eventUserState: has("eventUserState") ? plainObject(patch.eventUserState) : previous.eventUserState,
