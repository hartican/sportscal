...
function supabaseMaintenanceMode(environment = process.env){
  return /^(1|true|yes|on)$/i.test(cleanEnvironmentValue(environment.SUPABASE_MAINTENANCE_MODE));
}
...
function supabaseServiceRoleConfig(environment = process.env){
  ...
  const opaqueSecret = serviceRoleKey.startsWith("sb_secret_");
  return { ...config, serviceRoleKey, opaqueSecret, configured:Boolean(config.url && serviceRoleKey) };
}
...
async function supabaseRequest(path, {
  ...
  if (supabaseMaintenanceMode(environment)){
    throw new SupabaseRequestError("Supabase is temporarily offline for database recovery.", {
      status: 503,
      payload: { code: "supabase_maintenance" },
    });
  }
  const config = supabaseConfig(environment);
  ...
  const timeout = setTimeout(() => controller.abort(new Error("Supabase request deadline exceeded")), Math.max(250, Number(timeoutMs) || 8_000));
  try{
    ...
    if (controller.signal.aborted && !signal?.aborted){
      throw new SupabaseRequestError("Supabase did not respond before the request deadline.", {
        status: 504,
        payload: { code:"supabase_timeout" },
      });
    }
...
async function supabaseServiceRequest(path, options = {}){
  ...
  return supabaseRequest(path, {
    ...
    // Opaque sb_secret keys belong only in apikey. Legacy service_role JWTs
    // must also remain the bearer token until they are retired.
    accessToken:config.opaqueSecret ? "" : config.serviceRoleKey,
  });
}
...
function sanitizeEventUserState(value){
  const sanitized = Object.fromEntries(Object.entries(plainObject(value)).map(([key, action]) => {
    if (!plainObject(action, null)) return [key, action];
    const { mustWatch, mustWatchAddedAt, mustWatchSeenAt, ...safe } = action;
    return [key, safe];
  }));
  return eventActionIdentity.migrateActions(sanitized);
}
...
function sanitizePreferences(value){
  ...
  if (graph){
    ...
    if (Array.isArray(graph.entityFollows)) graph.entityFollows = graph.entityFollows.filter(item => !REMOVED_A_LEAGUE_TEAM_IDS.has(String(item?.participantId || item?.id || ""))).map(item => item?.followLevel === "mute" ? {...item,followLevel:"unfollow"} : item);
  }
  return preferences;
}
