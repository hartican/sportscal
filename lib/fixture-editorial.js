...
function createResolver(knowledge,events){
 ...
 return event=>{
  ...
  if(!projection)return locks.apply(event);
  ...
  return locks.apply({...event,editorialNarrative:enriched.editorialNarrative,editorialPreview:enriched.editorialPreview,storyline:enriched.storyline});
 };
}
