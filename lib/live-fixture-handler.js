...
const {readLiveSnapshots,refreshDueSources,overlaySnapshots}=require("./live-fixtures");
...
function createLiveFixtureHandler({read=readLiveSnapshots,refresh=refreshDueSources,sources,environment=process.env}={}){
  return async function handler(request,response){
    ...
    try{
      const snapshot=await read(),athlete=url.searchParams.get('athlete'),etag=`"fixtures:${snapshot.revision}${athlete?':'+crypto.createHash('sha256').update(athlete).digest('hex').slice(0,12):''}"`;
      response.setHeader("ETag",etag);
      if(!snapshot.stale&&(request.headers?.["if-none-match"]===etag||url.searchParams.get("revision")===snapshot.revision)){response.status(304).end();return;}
      const merged=overlaySnapshots([],snapshot.sources);
      if(athlete){
        if(!/^[a-z0-9:_-]{1,200}$/i.test(athlete)){response.status(400).json({error:'Invalid athlete identity'});return;}
        response.status(200).json({schemaVersion:'athlete-participation-live.v1',revision:snapshot.revision,stale:snapshot.stale,history:require('./athlete-participation').participationHistory(merged,athlete)});return;
      }
      ...
      const fixtures=merged.filter(event=>identity.retainedInActiveTimeline(event));
      response.status(200).json({schemaVersion:"live-fixtures.v1",revision:snapshot.revision,stale:snapshot.stale,sources:[{source_id:'current-fixtures',revision:snapshot.revision,fixtures},...snapshot.sources.map(({fixtures,...source})=>({...source,fixtures:[]}))]});
