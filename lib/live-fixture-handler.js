...
const {readLiveSnapshots,refreshDueSources}=require("./live-fixtures");
...
function createLiveFixtureHandler({read=readLiveSnapshots,refresh=refreshDueSources,sources=()=>require("./live-source-adapters").liveSources(),environment=process.env}={}){
  return async function handler(request,response){
    ...
    if(refreshRoute){
      ...
      try{const result=await refresh({sources:sources()});response.status(result.failed.length?207:200).json(result);}
    ...
    try{
      const snapshot=await read(),etag=`"fixtures:${snapshot.revision}"`;
      ...
      response.status(200).json({schemaVersion:"live-fixtures.v1",revision:snapshot.revision,stale:snapshot.stale,sources:snapshot.sources});
