...
function createLiveFixtureHandler({read=readLiveSnapshots,refresh=refreshDueSources,sources,publishedFixtures=()=>require('../data/follow-sources/coverage.v1.json').events,environment=process.env}={}){
  ...
  return async function handler(request,response){
    ...
    try{
      const snapshot=await read(),revision=crypto.createHash('sha256').update(`${snapshot.revision}:${libraryRevision}`).digest('hex'),athlete=url.searchParams.get('athlete'),etag=`"fixtures:${revision}${athlete?':'+crypto.createHash('sha256').update(athlete).digest('hex').slice(0,12):''}"`;
