...
async function rankings(query,user){
  ...
  const entries=[...events.values()].map(ev=>{
    const votes=byEvent.get(ev.id)||[],viewerHasRated=Boolean(user&&votes.some(row=>row.user_id===user.id));
    const ratingRequired=phase==='heat'&&!viewerHasRated;
    return {...ev,viewerHasRated,ratingRequired,crowd:ratingRequired?null:crowd.summary(votes,{phase,now})};
  }).sort((a,b)=>{
    if(a.ratingRequired!==b.ratingRequired)return a.ratingRequired?1:-1;
    if(a.crowd&&b.crowd)return crowd.compare(a,b);
    return Date.parse(a.startTimeUtc||'')-Date.parse(b.startTimeUtc||'')||a.id.localeCompare(b.id);
  });
