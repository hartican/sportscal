...
module.exports=async function marqueeModeHandler(request,response){
  ...
  try{
    ...
    const user=await admin(request),body=bodyOf(request),requestedEventId=clean(body.eventId),event=server.eventFor(requestedEventId),eventId=server.canonicalEventId(requestedEventId);
