...
async function discover({mode,athletes=[],fixtures=[],knownFixtures=fixtures,previous=[],now=new Date(),fetchImpl=globalThis.fetch,environment=process.env,signal}={}){
 ...
 if(!response.ok){let error;try{error=await response.json();}catch(_){}throw failure(error?.error?.code==='customer_verification_required'?'ai_billing_required':response.status===429?'ai_rate_limited':response.status===401||response.status===403?'ai_auth_unavailable':'ai_request_failed');}
