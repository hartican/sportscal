  ...
  return {
    async read({ids=[]}={}){
      ...
      try{
        ...
        return (manifest||[]).map(row=>({...row,fixtures:bySource.get(row.source_id)||[]}));
      }catch(error){
...
module.exports={TABLE,CURRENT_TABLE,refreshInterval,contentHash,overlaySnapshots,createSnapshotStore,refreshDueSources,readLiveSnapshots};
