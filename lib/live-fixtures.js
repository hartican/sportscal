...
const VOLATILE=new Set(["checkedAt","updatedAt","sourceCheckedAt","canonicalSourceCheckedAt","generatedAt","statusUpdatedAt","statusCheckedAt","scoreCheckedAt","participationCheckedAt"]);
...
function overlaySnapshots(events,snapshots){
  const updates=[...(snapshots||[])].filter(snapshot=>snapshot.source_id!=='discovery-ai-athletes').sort((a,b)=>(Date.parse(a.checked_at)||0)-(Date.parse(b.checked_at)||0)).flatMap(snapshot=>(snapshot.fixtures||[]).map(event=>snapshot.checked_at?{...event,sourceCheckedAt:snapshot.checked_at}:event));
