...
function eventMap(){
  ...
  (manifest.pages||[]).forEach(page=>{
    const doc=JSON.parse(fs.readFileSync(path.join(root,page.path),"utf8"));
    (doc.events||[]).forEach(event=>[event.canonicalEventId,event.eventId,event.id].filter(Boolean).forEach(id=>eventCache.set(String(id),event)));
  });
  return eventCache;
}
function eventFor(id){ return eventMap().get(String(id||""))||null; }
...
function decorated(row,identities){ return { ...row,userId:row.user_id,rating:Number(row.rating),tags:row.tags||[],updatedAt:row.updated_at,persona:identities.personas.get(row.user_id)?.persona||"general" }; }
...
async function snapshots(eventIds,{userId=null,detailId=null,now=new Date()}={}){
  const ids=[...new Set(eventIds.map(String).filter(id=>eventFor(id)))].slice(0,50);
  if (!ids.length) return [];
  const filter=`in.(${ids.join(",")})`,freshCutoff=new Date(now.getTime()-nsc.PRESENCE_TTL_MS).toISOString();
  const [contributions,likes,presence,sessions]=await Promise.all([
    rows(TABLES.contributions,{event_id:filter,select:"event_id,user_id,phase,bucket_start,rating,tags,created_at,updated_at"}),
  ...
  return ids.map(eventId=>{
    ...
    const contributorRows=eventId===detailId?(()=>{
    ...
    return {
      schemaVersion:nsc.SCHEMA_VERSION,eventId,phase,aggregate:aggregates[phase],aggregates,
      ...
      currentUser:userId?{contribution:current?{rating:current.rating,tags:current.tags||[],bucketStart:current.bucket_start}:null,liked:Boolean(currentLike),watching:presence.some(row=>row.event_id===eventId&&row.user_id===userId)}:null,
...
module.exports={TABLES,awardPoints,eventFor,eventMap,eventTiming,freezeSession,identityMaps,leaderboard,pilotFor,profileFor,personaFor,publicProfile,pulseSeedForEvent,rows,rowsPath,snapshots,sydneyMonday};
