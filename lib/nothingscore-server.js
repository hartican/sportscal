...
function eventMap(){
  ...
  (majorEventDocument.events||[]).filter(event=>event.kind!=="ticket_sale").forEach(parent=>{
    const parentEvent={
      ...
      startTimeUtc:null,
      timelineSortTimeUtc:parent.startDate?`${parent.startDate}T00:00:00.000Z`:null,
      timePrecision:parent.dateStatus==='tbc'?'tbc':'date-only',
    ...
    (parent.subEvents||[]).forEach(subEvent=>{
      const childEvent=eventWithTiming({
        ...
        startTimeUtc:subEvent.startTimeUtc||null,
        timelineSortTimeUtc:subEvent.timelineSortTimeUtc||subEvent.sessionStartTimeUtc||null,
        timePrecision:subEvent.timePrecision || (subEvent.startTimeUtc?'exact':subEvent.sessionStartTimeUtc?'follows':'tbc'),
      });
...
function eventAliasMap(){
  ...
  (majorEventDocument.events||[]).forEach(major=>{
    (major.subEvents||[]).forEach(subEvent=>{
      const start=feedControls.eventStart(subEvent)?.getTime();
...
let extraEventCache;
function eventFor(id){const canonical=canonicalEventId(id),found=eventMap().get(canonical);if(found)return found;if(!extraEventCache)extraEventCache=new Map(require('./calendar-catalogue').catalogue().map(ev=>[canonicalEventId(ev.eventId||ev.id),eventWithTiming(ev)]));return extraEventCache.get(canonical)||null;}
async function refreshEventSnapshots(ids=null){
  try{
    const {sources}=await require('./live-fixtures').readLiveSnapshots({ids:Array.isArray(ids)?ids:null});
    const events=require('./live-fixtures').overlaySnapshots([],sources);
    for(const update of events){const previous=eventFor(update.eventId||update.id),event=eventWithTiming({...previous,...update});for(const id of [update.id,update.eventId,update.canonicalEventId].filter(Boolean))eventMap().set(canonicalEventId(id),event);}
  }catch{/* Static and last-good fixtures remain available during a source/store outage. */}
}
function publicProfile(profile){
  if (!profile || profile.visibility!=="visible") return { displayName:"Hidden contributor", handle:null, avatarUrl:null, hidden:true };
  return { profileId:profile.profile_id, displayName:profile.display_name, handle:`@${profile.handle}`, avatarUrl:profile.avatar_url || null, hidden:false };
}
function ownerProfile(profile){
  if(!profile)return null;
  if(profile.visibility==="deleted")return{profileId:profile.profile_id,displayName:null,handle:null,avatarUrl:null,visibility:"deleted",hidden:true,deleted:true};
  return{
...
function activeRealContributors90d(contributions,_pilots,now=new Date()){
  ...
  (contributions||[]).forEach(row=>{
    ...
    if(userId&&validPhase&&submitted&&Number.isInteger(rating)&&rating>=1&&rating<=5&&Number.isFinite(at)&&at>=start&&at<=end)active.add(userId);
  });
  return active.size;
}
async function earlyPanelStatus(now=new Date()){
  ...
  const [stateRows,contributions]=await Promise.all([
  ...
  const state=stateRows[0]||null,active=activeRealContributors90d(contributions,[],reference),threshold=Math.max(1,Number(state?.retirement_threshold)||EARLY_PANEL_THRESHOLD);
  return{
...
function eventTiming(event){ return { status:event?.status, startTimeUtc:event?.startTimeUtc,endTimeUtc:event?.endTimeUtc,liveWindowHours:Number(event?.liveWindow||3) }; }
...
async function snapshots(eventIds,{userId=null,detailId=null,now=new Date(),demoMode=process.env.NSC_DEMO_PANEL_MODE,internalDemo=false,editorialResearch=false}={}){
  ...
  const panelMode=demoPanel.mode(demoMode);
  // Independent metadata must not hold up the bounded fixture reads.
  const [panelStatus,contributions,likes,presence,sessions,viewerPoints]=await Promise.all([
    panelMode==="off"?Promise.resolve({configured:false,publicEnabled:false,activeRealContributors90d:0,retirementThreshold:EARLY_PANEL_THRESHOLD,eligibleToRetire:false,retiredAt:null,updatedAt:null}):earlyPanelStatus(now),
  ...
  const includesModelled=internalDemo && modelledPanelEnabled(panelMode,{internal:true,status:panelStatus});
  ...
  return requests.map(({requestedId,eventId})=>{
    ...
    const phase=crowd.phaseFor({...eventTiming(event),session},now);
    ...
    const realAggregates=Object.fromEntries(['heat','pulse','impact'].map(phase=>{const result=crowd.summary(eventRows,{phase,now});return [phase,{score:result.average,ratingCount:result.count,contributorCount:result.count,building:result.early,support:result.count}];}));
    ...
    const activeRows=crowd.latestVotes(eventRows,{phase,now});
    ...
    const realAggregateVisible=uniqueContributors(activeRows)>=1,displayRows=includesModelled?[...activeRows,...activeDemoRows]:realAggregateVisible?activeRows:[];
    ...
    const viewerHasRated=Boolean(currentContribution);
    const ratingRequired=phase==="heat"&&!viewerHasRated;
    const aggregateAllowed=!ratingRequired;
    return {
      ...(editorialResearch?{fiveStarPhases:["heat","pulse","impact"].filter(phase=>crowd.latestVotes(eventRows,{phase,now}).some(row=>Number(row.rating)===5))}:{}),
      peerResults:aggregateAllowed?crowd.summary(eventRows,{phase,now}):null,
      replayResearchRequested:phase==="impact" && crowd.latestVotes(eventRows,{phase:"impact",now}).some(row=>Number(row.rating)===5),
      schemaVersion:nsc.SCHEMA_VERSION,eventId:requestedId,canonicalEventId:eventId,phase,viewerHasRated,ratingRequired,
      aggregate:aggregateAllowed?{...displayAggregate,contributorMix:mixes[phase]}:null,
      filterRatings:require('./feed-filter-ratings')(eventRows.filter(row=>!identities.personas.get(row.user_id)?.moderation_flag),userId,{crowdAllowed:aggregateAllowed}),
      aggregates:{...aggregates,...(phase==="heat"&&!aggregateAllowed?{heat:null}:{})},
      ...(eventId===detailCanonicalId&&aggregateAllowed?{series}:{}),crowdEditorial:aggregateAllowed?editorial:null,
      ...
      contributors:aggregateAllowed?contributorRows:undefined,
