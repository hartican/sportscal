...
function picks(state){
 ...
 for(const x of p.preferenceGraph?.entityFollows||[])if(!['none','mute','unfollow'].includes(x.followLevel))items.push(item(x.participantId,'entity'));
...
function excluded(state,pick){
 ...
 if(pick.kind==='entity')return p.preferenceGraph?.entityFollows?.some(x=>x.participantId===pick.id&&['mute','unfollow'].includes(x.followLevel));
