...
function catalogue(){
  ...
  const enrich=require('./fixture-editorial').createResolver(require('../data/editorial-knowledge.v1.json'),events);
  cached=fixtureIdentity.estimateTimeline(fixtureIdentity.mergeOverlays([...byIdentity.values()],require('../data/discovery/enrichment.v1.json').events)).map(enrich);return cached;
}
function subscriptionEvents(state,selection,now=new Date(),{historyDays=7,snapshots=[]}={}){
  ...
  const eligible=events.filter(event=>{
    ...
    return !followPolicy.explicitlyExcluded(event,preferences) && (participantFollow || Boolean(followFirst.reasonForEvent(event,preferences)));
  }).map(calendar.idFor);
