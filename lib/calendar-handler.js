...
function selectionIds(value){
  ...
  return calendar.migrateSelectionIds(value);
}
module.exports=async function calendarHandler(request,response){
  ...
  try{
    ...
    if(method==='GET' && token){
      ...
      const live=await require('./live-fixtures').readLiveSnapshots().catch(()=>null);
      const events=subscriptionEvents(userStateFromRow(states[0]),{includedIds:subscription.included_ids,excludedIds:subscription.excluded_ids},new Date(),{snapshots:live?.sources || []});
      if(live)response.setHeader('X-Fixture-Revision',live.revision);
    ...
    if(method==='GET'){
      ...
      const subscription=rows?.[0] ? {...rows[0],included_ids:selectionIds(rows[0].included_ids || []),excluded_ids:selectionIds(rows[0].excluded_ids || [])} : null;
      response.status(200).json({subscription});return;
    }
    if(method==='DELETE'){
