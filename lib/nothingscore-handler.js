...
async function uploadAvatar(body,user){
  ...
  if(!match)throw new NscError("Choose a JPEG, PNG or WebP profile picture.",400,"invalid_profile_avatar");
  const bytes=Buffer.from(match[2],"base64");
  if(!bytes.length||bytes.length>1024*1024)throw new NscError("Profile pictures must be 1 MB or smaller after cropping.",413,"profile_avatar_too_large");
  const mime=match[1]==="jpeg"?"image/jpeg":`image/${match[1]}`,extension=match[1]==="jpeg"?"jpg":match[1];
  const objectPath=`${user.id}/${Date.now()}-${crypto.randomUUID()}.${extension}`;
  const config=supabaseConfig(),service=supabaseServiceRoleConfig();
  const upload=await fetch(`${config.url}/storage/v1/object/nothingsports-profile-avatars/${objectPath}`,{method:"POST",headers:{Authorization:`Bearer ${service.key}`,apikey:service.key,"Content-Type":mime,"x-upsert":"false"},body:bytes});
  if(!upload.ok)throw new NscError("The profile picture could not be stored.",502,"profile_avatar_upload_failed");
  const avatarUrl=`${config.url}/storage/v1/object/public/nothingsports-profile-avatars/${objectPath}`;
  await supabaseServiceRequest(server.rowsPath(server.TABLES.profiles,{user_id:`eq.${user.id}`}),{method:"PATCH",headers:{Prefer:"return=minimal"},body:{avatar_url:avatarUrl,updated_at:new Date().toISOString()}});
  return {avatarUrl};
}
...
async function pulseAction(body,user,access,now){
  ...
  const rating=Number(body.rating);if(!Number.isInteger(rating)||rating<1||rating>5)throw new NscError('Choose a rating from 1 to 5.',400,'invalid_rating');
  return require('./nsc-rewards').submit(user.id,id,'pulse',rating,event,now);
}
...
module.exports=async function nothingscoreHandler(request,response){
  ...
  try{
    ...
    if((request.method||"GET")==="GET"){
      if(query(request,'ladder')){response.status(200).json(await require('./nsc-ladder').ladder({cursor:query(request,'cursor')},user));return;}
      ...
      await server.refreshEventSnapshots(ids);
      const snapshots=await server.snapshots(ids,{userId:user?.id||null,detailId:detailId||null,now:new Date()});
      const myProfile=user?await server.profileFor(user.id):null,persona=user?await server.personaFor(user.id):null;
    ...
    else if(body.action==="follow-user")result=await require("./user-follows").setFollow(body,user);
    ...
    if(error instanceof NscError || error.status===400){response.status(error.status).json({error:error.message,code:error.code});return}
