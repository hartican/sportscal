...
async function ratingAction(body,user,_access,now){
  ...
  if(!["heat","impact"].includes(requestedPhase))throw new NscError("Choose Anticipation or Impact before submitting.",400,"invalid_nsc_phase");
  ...
  if(requestedPhase!==state.phase){
    const prior=(await server.rows(server.TABLES.contributions,{event_id:`eq.${id}`,user_id:`eq.${user.id}`,phase:`eq.${requestedPhase}`,bucket_start:"eq.1970-01-01T00:00:00.000Z",submitted_at:"not.is.null",select:"contribution_id",limit:"1"}))[0]||null;
    if(!prior)throw new NscError("That scoring phase has ended. Refresh before submitting.",409,"phase_action_mismatch");
  }
  try{return await server.submitRating(user.id,id,requestedPhase,rating,tags,now.toISOString())}
  catch(error){
    if(error instanceof SupabaseRequestError&&[error.message,error.payload?.message,error.payload?.details].some(value=>String(value||"").includes("nsc_already_submitted")))throw new NscError("Anticipation or Impact has already been submitted for this fixture.",409,"nsc_already_submitted");
    throw error;
  }
}
...
module.exports=async function nothingscoreHandler(request,response){
  ...
  try{
    ...
    if((request.method||"GET")==="GET"){
      ...
      response.status(200).json({schemaVersion:nsc.SCHEMA_VERSION,snapshots,...(detailId?{detail:snapshots[0]||null}:{}),viewer:user?{signedIn:true,pilot:Boolean(pilot?.approved&&!pilot.suspended),profile:server.ownerProfile(myProfile)}:{signedIn:false,pilot:false,profile:null}});return;
    }
