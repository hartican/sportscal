...
function calendarSportSelectionMatchesEvent(selection, event){
  const selectedKeys = selection instanceof Set ? selection : new Set(selection || []);
  const eventKey = String(event?.key || "").toLowerCase();
  if (selectedKeys.has(eventKey)) return true;
  const eventNodeId = discoveryCatalogue.eventNodeId(event);
  if (!eventNodeId) return false;
  return Array.from(selectedKeys).some(key => {
    const selectedNodeId = discoveryCatalogue.eventNodeId({ key });
    return Boolean(
      selectedNodeId
      && discoveryCatalogue.familyIds(selectedNodeId).includes(eventNodeId)
    );
  });
}
...
function filterCalendarEvents(events, config, now = new Date()){
  ...
  return (Array.isArray(events) ? events : [])
    .filter(event => calendarSportSelectionMatchesEvent(sports, event))
    .filter(event => (
    ...
    .filter(event => {
      if (calendarSportSelectionMatchesEvent(allFixtures, event)) return true;
      if (Number(event.stakesScore || event.expected || 0) >= Number(config.minStakes || 3)) return true;
      return (
        calendarSportSelectionMatchesEvent(majorFollowedSports, event)
