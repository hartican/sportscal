...
function filterCalendarEvents(events, config, now = new Date()){
  ...
  return (Array.isArray(events) ? events : [])
    ...
    .filter(event => {
      if (allFixtures.has(String(event.key || "").toLowerCase())) return true;
      return Number(event.stakesScore || event.expected || 0) >= Number(config.minStakes || 3);
    })
...
function buildCalendarIcs(events, {
  ...
  reminderMinutes = 60,
  calendarName = "nothingSports",
} = {}){
  const stamp = calendarTimestamp(generatedAt);
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//nothingSports//Calendar sync//EN",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
    `X-WR-CALNAME:${escapeCalendarText(calendarName)}`,
    "X-WR-CALDESC:Your live customised nothingSports recommendations",
  ...
  events.forEach(event => {
    ...
    if (reminderMinutes > 0){
      lines.push(
        ...
        "ACTION:DISPLAY",
        `DESCRIPTION:${escapeCalendarText(`nothingSports: ${event.spoilerSafeTitle || event.displayTitleCompact || event.name}`)}`,
        "END:VALARM"
      );
    }
