...
function parseCricketPage(html,{sourceUrl,checkedAt=new Date().toISOString()}={}){
  ...
  return games.filter(game=>game?.id).map(game=>{
    ...
    return identity.normalizeCore({id,eventId:id,canonicalEventId:id,sourceFixtureId:providerId,key:"cricket",sport:"Cricket",sportDomainId:"sport:cricket",
      ...
      scoreDisplay:game.isCompleted?(require('../data/canonical/current-card-evidence-2026.json').resultOverrides.find(result=>[result.id,result.canonicalId].includes(id))?.scoreDisplay || game.resultText || null):null,innings:game.innings||[],format:game.gameType||null,numberOfDays:game.numberOfDays,
