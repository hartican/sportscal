...
function fixtures(){
  ...
  const finals=phases.flatMap(phase=>(phase.fixtures||[]).map(fixture=>{
    ...
    return identity.normalizeCore({...base,id,eventId:id,canonicalEventId:id,
      ...
      status:base.status||'scheduled',timePrecision:base.startTimeUtc?'exact':base.schedulePrecision==='week'?'week':'unconfirmed',
      schedulePrecision:base.startTimeUtc?'exact':base.schedulePrecision||'unconfirmed',
      weekAnchorDate:base.weekAnchorDate||null,displayDateLabel:base.displayDateLabel||null,
