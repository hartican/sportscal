(function(root){
 ...
 function copy(item,spoilers){
  ...
  switch(item.kind){
   case 'chat':return {title:item.messageCount>1?`${item.messageCount} new messages in ${room}`:`${item.sender||actor} sent a message in ${room}`,detail:spoilers&&item.preview?`${item.sender||actor}: ${item.preview.slice(0,140)}`:'Open the conversation to read.'};
   ...
   case 'points':return {title:`You earned ${item.points} point${item.points===1?'':'s'}`,detail:({heat_rating:'You rated the anticipation.',pulse_rating:'You rated the live action.',impact_rating:'You rated the impact.',foresight_bonus:'Your prediction matched another fan’s rating.',follow_person:'You followed someone new.'})[item.detail]||'View your points and rewards.'};
   case 'rating':return {title:`${actor} rated a fixture 5/5`,detail:`${({heat:'Heat',pulse:'Live',impact:'Impact'})[item.phase]||'Rating'} · open their pick.`};
   case 'reminder':return {title:item.title||'Your sporting reminder',detail:'Your reminder is due. Open the fixture.'};
 ...
 function create({request,owner,spoilers,navigate,closeDestination,signIn}){
  ...
  let account='',items=[],cursor=null,count=0,generation=0,timer=null,busy=false,opened=false,position=0,readTimer=null,refreshTimer=null,lastFetch=0;
  ...
  async function refresh(older=false){
   ...
   busy=true;lastFetch=Date.now();const ticket=generation,who=account;
   if(opened&&!items.length)render();
   try{
    const data=await request({cursor:older?cursor:'',summary:!opened});if(ticket!==generation||who!==owner())return;
   ...
   }catch(error){console.warn('Inbox refresh failed',error);if(ticket===generation&&opened){status.textContent='Notifications couldn’t load. ';const retry=node('button','Retry','btn ghost');retry.onclick=()=>void refresh(older);status.append(retry);}}
   finally{if(ticket===generation){busy=false;clearTimeout(timer);if(opened&&!document.hidden)timer=setTimeout(refresh,30000);}}
  }
  function show(push=true){
   if((owner()||'')!==account)reset();
   if(opened)return;generation++;busy=false;opened=true;if(push)history.pushState({...history.state,inbox:true},'');
  ...
  function hide(){opened=false;position=body.scrollTop;observer.disconnect();clearTimeout(timer);clearTimeout(readTimer);pending.clear();dialog.close();document.body.classList.remove('notifications-open');trigger.setAttribute('aria-expanded','false');trigger.focus({preventScroll:true});}
  ...
  addEventListener('popstate',()=>{if(history.state?.inbox){closeDestination();show(false);}else if(opened)hide();});
