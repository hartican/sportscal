...
globalThis.openFeedViewFilter=()=>{
 ...
 apply.onclick=async()=>{apply.disabled=true;status.textContent='Loading your Feed…';try{
  await loadAllFeedPages();if(!dialog.isConnected)return;
  if(Number(rating.value)){const context=buildFollowEligibilityContext(),ids=activeEvents.filter(e=>eventFollowReason(e,context)).map(nothingscoreEventId);for(let i=0;i<ids.length;i+=50){if(!dialog.isConnected)return;await loadNothingscoreBatch(ids.slice(i,i+50),{rerender:false});}}
  if(!dialog.isConnected)return;feedViewFilters={sport:sport.value,minimum:Number(rating.value)};feedFilterHydratedKey=`${serverSyncClient?.sessionSubject()||'public'}|${feedViewFilters.sport}|${feedViewFilters.minimum}`;localStorage.setItem('ns-feed-view-filter-v1',JSON.stringify(feedViewFilters));dialog.close();renderAll({preserveViewport:true});
