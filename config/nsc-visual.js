(function attachNscVisual(root, factory){
  ...
  function build(snapshot, { phaseName = "Build-up" } = {}){
    ...
    [["real","Real contributors"],["demo","Demo panel"]].forEach(([kind,label]) => {
    ...
    if (phase === "pulse"){
      ...
      const realPoints=points("realScore"),demoPoints=points("demoScore");
      if(realPoints)add("polyline",{points:realPoints,class:"nsc-series-real"});
      if(demoPoints)add("polyline",{points:demoPoints,class:"nsc-series-demo"});
    } else {
      rows.forEach((row,index)=>{
        const x=35+index*53,realHeight=(Number(row.realPercent)||0)*.72,demoHeight=(Number(row.demoPercent)||0)*.72;
        add("rect",{x:x-10,y:100-realHeight,width:16,height:realHeight,class:"nsc-bar-real",rx:3});
        add("rect",{x:x+8,y:100-demoHeight,width:16,height:demoHeight,class:"nsc-bar-demo",rx:3});
        const percentage=add("text",{x:x+7,y:Math.max(12,96-Math.max(realHeight,demoHeight)),"text-anchor":"middle",class:"nsc-percent-label"});
    ...
    if(phase==="pulse"){
      table.innerHTML += "<thead><tr><th>Bucket</th><th>Score</th><th>Real contributors</th><th>Demo contributors</th></tr></thead>";
      const body=document.createElement("tbody");
      rows.forEach(row=>{const tr=document.createElement("tr");[new Date(row.bucketStart).toLocaleTimeString("en-AU",{hour:"numeric",minute:"2-digit"}),row.score??"Building",row.realContributors,row.demoContributors].forEach(value=>{const td=document.createElement("td");td.textContent=String(value);tr.appendChild(td)});body.appendChild(tr)});
      table.appendChild(body);
    }else{
      table.innerHTML += "<thead><tr><th>Rating</th><th>Real contributors</th><th>Real percent</th><th>Demo contributors</th><th>Demo percent</th><th>Displayed percent</th></tr></thead>";
      const body=document.createElement("tbody");
      rows.forEach(row=>{const tr=document.createElement("tr");[row.rating,row.realCount,`${row.realPercent}%`,row.demoCount,`${row.demoPercent}%`,`${row.displayPercent}%`].forEach(value=>{const td=document.createElement("td");td.textContent=String(value);tr.appendChild(td)});body.appendChild(tr)});
      table.appendChild(body);
    }
