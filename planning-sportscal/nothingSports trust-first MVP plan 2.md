...
7. Run a 14-day trust pilot before considering social.

Current evidence:

- The preference, canonical-sports and feed validators pass, but the main NS verifier fails because existing profiles receive only 25 of 34 Commonwealth Games cards.
- Six finished events currently lack results.
- The published feed has 304 cards and canonical data has 218 AFL and 204 NRL fixtures.
- A clean-profile browser check shows “Rugby League selected” renders only nine cards despite 31 future routine NRL fixtures being present.
- Desktop and 390px layouts currently have no horizontal overflow or console warnings.
- The uncommitted taxonomy/feed expansion is intentional and must be preserved and stabilised.
...
- The normal app remains usable without telemetry. Participation in the closed pilot requires sign-in and acknowledgement of the measurement disclosure.
...
### Phase 6 — Fourteen-day pilot and decision gate
...
- Review TSDR, full-fixture adoption, cross-check rate, regret reports, card density, prompt burden and qualitative trust responses by curator, hybrid and completist cohorts.
- Use the pilot readout to choose the next investment:
  - improve coverage/data freshness if trust remains weak;
  - deepen personalisation if coverage is trusted but relevance is weak;
  - consider `Watching Now` only if the core decision loop is already working.
