...
## Compact Feed polish — 25 September 2026

Keep the short hook visible. Target 20–25% less height on equivalent 390px full cards through spacing and shared rows, retaining 44px flame/provider/profile/action targets. Use a competition header between top controls, equal opponent columns with 96px contained logo frames (80px on the narrowest phones), prominent timing and one wrapping stage, ratings/community summary, transparent provider row, venue, hook, then a wrapping More/Remind/Chat footer. Compact cards share the order with 48px identity frames. Full cards, compact rows and lists use contextual sizes, not one literal pixel dimension across the app. No added decorative motion.

Display labels are curated by canonical identity: Dockers/Lions and established national nicknames such as Socceroos/Matildas/Proteas, retaining gender distinctions. Canonical names, aliases and IDs remain unchanged. Preserve unresolved/spoiler-protected labels; neither nicknames nor team colour tints may expose hidden opponents. Palettes use reviewed club or national sporting colours for team contests, tournament branding for event-led tournaments and canonical venue-country sporting colours for motorsport. Unknown palettes remain neutral; featured athlete pairs never change event format.

Tennis tournament branding and the readable tournament label form the existing schedule navigation link in the top header. The duplicate footer link is removed. Existing additional schedule disclosure stays available. Real transparent symbols are preferred; retain full crests with integral lettering if no verified alternative is available. Manual replacement covers current Feed/supplied examples only. Source notes: `assets/identities/events/feed-polish-sources.md` and `assets/providers/README.md`. No historical asset audit, invented artwork or photography pipeline.

Preserve editorial locks, ratings/spoilers, profile and schedule navigation, reminder/chat actions, dismissal and scroll/focus restoration. Asset dimensions reserve space during decoding and fallbacks. App-shell and asset-cache versions advance together. Normal release/performance/PWA gates remain required; prior release exceptions are not carried forward.

## 25 September 2026 — identity and tournament corrections

Visual direction: restrained sporting colour, balanced contained artwork and readable links. The order remains identity/time, rating, providers, venue, left-aligned editorial and actions. No additional decorative motion; existing expansion and reduced-motion behaviour remain.

Team labels are centred and actionable labels underlined. Brazil uses the approved display label “Brazil · Canarinho”; canonical profiles and accessible names are unchanged. Socceroos receives a full-card-only optical inset. The supplied Cricket Australia PNG is copied byte-for-byte, with its lettering, transparency and SHA recorded in the shared registry; gender identities remain separate. User attachment provenance is distinct from an official download.

Reviewed club palettes use their primary colour; matching primary colours use a documented secondary. Premier League club IDs map to familiar home shirt/crest colours (red/white, blue/white, claret/sky-blue, black/white and gold/black), rather than deriving colours from participant names. Socceroos green opposes Brazil gold. Unknown identities remain neutral. Host colour pairs use national sporting conventions (Australia green/gold, Brazil green/gold, Italy green/red, Netherlands orange/gold, Malaysia gold/charcoal), otherwise restrained national flag colours. Azerbaijan uses red/green plus a low-opacity blue accent. Presidents Cup uses charcoal/gold. These are design choices, not fixture facts or team participation.

F1 reviewed sources are the 2026 race hubs recorded by lib/f1-venues.js. The official Bahrain hub now describes the Bahrain Grand Prix in Malaysia at Sepang: preserve the round identity while using Malaysia host metadata. No venue is inferred from a driver.

Tournament overviews are full before/on day one, compact from day two during their sourced date range or while confirmed active, and retain separate underlined Follow navigation and inline expansion. Child cards retain their usual size and admission. Athlete photography and further asset acquisition remain deferred.

Local verification: 320/390/768/1280px, both themes, with decoded cricket/F1/football marks, centred links and expanded/compact parents. Presidents Cup Schedule pin actions retain full cards; Feed-only parent compaction does not remove them. The browser parent filter and direct-event schedule loading preserve the overview and explicitly followed sessions. Last-good schedule caches remain available after a failed fetch. Release remains subject to normal canonical, performance and installed-PWA gates; this record is not production evidence.
